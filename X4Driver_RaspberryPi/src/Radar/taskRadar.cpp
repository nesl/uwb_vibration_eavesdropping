...
int taskRadar(void)
{
...
#endif
    status = x4driver_set_iterations(x4driver, 6);
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
...
#endif
    status = x4driver_set_pulses_per_step(x4driver, 12);
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
    ...
    status = x4driver_set_fps(x4driver, 1000); // Generate 5 frames per second
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
